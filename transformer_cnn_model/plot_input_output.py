...
----------------------------------------------------------------------
Select a specific year and reach:
...
The script then:
  • Infers the required temporal depth T from config.py (year_target - 1)
  • Automatically loads the previous T years of images
  • Uses the specified year as the prediction target
  • Runs the model and visualizes inputs, prediction, and target
...
# ---------------------------------------------------------
# Checkpoint resolution (same logic)
# ---------------------------------------------------------
...
# Plot helper
# ---------------------------------------------------------
def _imshow(ax, img2d: np.ndarray, title: str, vmin=None, vmax=None):
    im = ax.imshow(img2d, vmin=vmin, vmax=vmax)
    ax.set_title(title)
    ax.axis("off")
    return im
...
def plot_inputs_pred_target(
    ...
    T = inputs_np.shape[0]
    assert T == 4, "This layout assumes exactly 4 input images."

    # Binary prediction
    pred_bin = (pred_probs_np >= threshold).astype(np.float32)

    # Grey for 0 (no water), white for 1 (water)
    grey_white = ListedColormap([(0.6, 0.6, 0.6), (1.0, 1.0, 1.0)])

    fig = plt.figure(figsize=(14, 9))

    # -------------------------
    # Layout parameters (tune if needed)
    # -------------------------
    left_margin = 0.06
    right_margin = 0.06
    ...
    # Make an explicit "gap band" between rows for bracket + label
    gap_band_h = 0.1
    label_to_bottom_gap = 0.06   # NEW: empty space between label and prediction row
    ...
    # Bottom grouping: prediction + target centered under inputs
    bottom_panel_w = 0.23
    gap_x_bottom = 0.05
    ...
    for i in range(4):
        ...
        ax.imshow(inputs_np[i], cmap=grey_white, vmin=0.0, vmax=1.0, interpolation="nearest")
    ...
    ax_pred.imshow(pred_bin, cmap=grey_white, vmin=0.0, vmax=1.0, interpolation="nearest")
    ax_pred.set_title("Prediction", fontsize=18, pad=10)
    ax_pred.axis("off")

    ax_tgt.imshow(target_np, cmap=grey_white, vmin=0.0, vmax=1.0, interpolation="nearest")
    ax_tgt.set_title("Target", fontsize=18, pad=10)
    ax_tgt.axis("off")

    # -------------------------
    # Bracket + label in the GAP BAND (separate line)
    # -------------------------
    left = input_axes[0].get_position().x0
    right = input_axes[-1].get_position().x1

    # Place bracket high in the gap band so it doesn't clash with bottom titles
    ...
    # Label under bracket, still inside the gap band (and NOT near Prediction/Target titles)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot input sequence + prediction + target (year-based or sample-based)."
    )

    parser.add_argument("--epoch", type=int, default=None,
                        help="Epoch checkpoint to load; else latest.")
    parser.add_argument("--year", type=int, default=None,
                        help="Target year (activates year-based mode).")
    parser.add_argument("--reach", type=int, default=None,
                        help="Reach ID (required if --year is used).")
    parser.add_argument("--split", type=str, default="testing",
                        choices=["training", "validation", "testing"],
                        help="Dataset split (year-based mode).")

    parser.add_argument("--sample", type=int, default=0,
                        help="Test sample index (used if --year not given).")

    parser.add_argument("--threshold", type=float, default=train_cfg.water_threshold,
                        help=f"Water threshold (default {train_cfg.water_threshold}).")

    parser.add_argument("--save", action="store_true",
                        help="Save instead of display.")
...
def main():
    ...
    if args.year is not None:
        ...
        with torch.no_grad():
            pred = model(x)  # (1,1,H,W) or (1,H,W) depending on model
        pred = pred.squeeze().detach().cpu().numpy()  # (H,W)

        title = f"Prediction – year {args.year}, reach {args.reach}, split {args.split} ({model_type})"

        if args.save:
            out_name = f"prediction_year{args.year}_reach{args.reach}_{args.split}_{model_type}.png"
    ...
    if args.save:
        out_name = f"prediction_sample{sample_idx:03d}_{model_type}.png"
        out_path = data_cfg.plots_dir / out_name
    else:
        out_path = None

    plot_inputs_pred_target(
        inputs_np=x.squeeze(0).detach().cpu().numpy(),  # (T,H,W)
        pred_probs_np=pred,
        target_np=y_np,
        threshold=threshold,
        save_path=out_path,
