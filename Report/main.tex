...
\begin{abstract}
    \noindent This project investigates whether transformer-based architectures can improve the prediction of morphological changes in the Brahmaputra River from multi-year satellite imagery. We compare a hybrid Transformer–CNN model (TransUNet) and a full window-based transformer model (st-Swin-UNet) against the JamUNet CNN baseline. Despite explicitly modelling temporal dependencies, neither transformer-based approach outperforms the CNN in terms of F1 score, precision, or recall. Our results suggest that limited training data, coupled with the inherently chaotic dynamics of braided rivers, constrain the effectiveness of transformer architectures in this setting. All related code is given in the following \href{https://github.com/Romultra/River-Morpho-ML.git}{link}.
\end{abstract}
...
\\
Comparing st-Swin-UNet with JamUNet (F1 $= 0.712$), we observe a negligible performance gap. While st-Swin-UNet (best: F1 $= 0.7044$) outperforms TransUNet (epoch 19 F1 $= 0.684$) by $2.9\%$, it still falls short of the simple CNN baseline. The window-based attention mechanism successfully addresses the memory efficiency issues of TransUNet's per-pixel temporal attention, allowing for larger models ($41.3$M parameters vs $500$K), yet this architectural sophistication does not translate to improved prediction accuracy. The hierarchical feature extraction and learnable temporal position embeddings capture spatial patterns effectively, as evidenced by comparable accuracy ($92.8\%$ vs $93.5\%$), but struggle with precise boundary delineation, resulting in lower precision ($67.1\%$ vs $72.6\%$) and recall ($74.2\%$ vs $70.2\%$) compared to JamUNet.


\section{Evaluation and conclusion} \label{section: Evaluation and conclusion}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Variation of test loss against training epoch (4 year input).}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{F1, recall, precision, csi scores comparison between TransUNet and JamUNet models (4 year input).}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Misclassification map comparison between TransUNet and JamUNet models on a selected test image (4 year input).}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/prediction_stswin_small_4y_sample001.png}
