...
While TransUNet introduces explicit temporal modelling through per-pixel transformers, it faces a fundamental computational challenge: processing each pixel's temporal sequence independently results in very large sequence counts ($H \times W$ sequences of length $T$), requiring chunked processing to avoid memory overflow. This motivates the investigation of window-based self-attention mechanisms, specifically the Swin Transformer architecture \cite{liu2021swin}, which has demonstrated strong performance in hierarchical vision tasks while maintaining computational efficiency through localised attention windows. \\
\\
The st-Swin-UNet (spatio-temporal Swin-UNet) model replaces the entire JamUNet CNN backbone with a Swin Transformer-based U-Net architecture. Unlike TransUNet, which applies transformers only to the temporal dimension before feeding into a spatial CNN, st-Swin-UNet employs window-based self-attention for both spatial feature extraction and implicit spatio-temporal modelling. This design choice is inspired by recent video understanding architectures \cite{liu2022video}, which show that windowed attention mechanisms can efficiently process spatio-temporal data. \\
\\
The architecture begins with a spatio-temporal patch embedding module that processes the input sequence $x \in \mathbb{R}^{B \times T \times H \times W}$. Each temporal frame is embedded separately using a shared convolutional projection layer (patch size $4 \times 4$), producing spatial patches. Crucially, learnable temporal position embeddings are then added to each frame's patch embeddings before temporal aggregation. These temporal position embeddings allow the model to distinguish between different years in the input sequence and learn their relative importance for prediction. The temporal frames are aggregated using a \textit{concat-proj} strategy: all frame embeddings are concatenated and projected back to the target dimension through a two-layer MLP, providing an expressive temporal fusion mechanism. \\
\\
Following patch embedding, st-Swin-UNet employs a symmetric encoder-decoder architecture with four hierarchical stages. The encoder progressively downsamples spatial resolution (by factors of 2, 4, 8, 16) while increasing feature dimensionality, using torchvision's \texttt{SwinTransformerBlock} and \texttt{PatchMerging} modules. Each stage contains multiple Swin Transformer blocks with window-based self-attention (window size $8 \times 8$ for tiny variant). Unlike global self-attention, which has quadratic complexity $\mathcal{O}((HW)^2)$, window-based attention computes attention only within non-overlapping local windows, reducing complexity to $\mathcal{O}(HW)$ and making the approach tractable for our $1000 \times 500$ images. Shifted window partitioning between consecutive blocks enables cross-window connections while maintaining efficiency. \\
\\
The decoder mirrors the encoder structure, progressively upsampling features through custom \texttt{PatchExpanding} layers (the inverse operation of \texttt{PatchMerging}) and concatenating skip connections from corresponding encoder stages. This U-Net structure with skip connections preserves fine-grained spatial details lost during downsampling, which is critical for accurate pixel-level segmentation of narrow river channels. A final head consisting of two transposed convolutions upsamples the output to the original resolution and produces the binary prediction. A block diagram of the st-Swin-UNet architecture is shown in Figure \ref{fig: StSwinUNet block diagram}.
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[
        node distance=1.0cm,
...
\end{figure}
\noindent Two model variants were developed: the \textit{tiny} variant (embedding dimension 48, depths [2,2,2,2], $\sim$6.8M parameters) and the \textit{small} variant (embedding dimension 96, depths [2,2,6,2], $\sim$11M parameters). Despite the larger parameter count compared to TransUNet and JamUNet ($\sim$500K), window-based attention makes st-Swin-UNet more memory-efficient than per-pixel temporal transformers, requiring 8--10 GB GPU memory with batch size 8 for the tiny variant. \\
\\
To summarise, st-Swin-UNet differs fundamentally from both JamUNet and TransUNet. Unlike JamUNet's shallow 3D convolutions with fixed local receptive fields, st-Swin-UNet employs hierarchical window-based attention that can model both spatial features and temporal patterns through learned position embeddings. Unlike TransUNet's per-pixel temporal attention followed by CNN, st-Swin-UNet replaces the entire backbone with transformers, using window-based attention to achieve computational tractability. The architecture is expected to benefit from the hierarchical feature extraction and global receptive field of transformers while avoiding the memory bottlenecks of global attention, potentially improving predictions of complex morphological patterns spanning multiple spatial scales.
